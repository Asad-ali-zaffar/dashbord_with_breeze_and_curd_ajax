(function($){
   ...
   var table=$('#table').DataTable( {
        ...
        fixedHeader: true,
        "ajax": {
            url: db_url+"get_customer_price_list"
        },
        // orderCellsTop: true,
        fixedHeader: true,
        "columns": [
   ...
   //active
   $('#customer_price_list').addClass('active');
   ...
   //delete patient
    $(document).on('click','.delete_patient',function(e){
        ...
        var el=$(this);
        swal({
            title: trans("Are you sure to delete customer price list ?"),
            type: "warning",
...
$(document).on('click','.add_centers',function (e){
	...
	$.ajax({
		...
		type: "POST",
		data: { "test_id": $('#test_id').val() , "status": $('#status').val() , "cust_price_list_id": cust_price_list_id  , "discount_type": $('#discount_type').val()  , "price": $('#price').val() , "party_price": $('#party_price').val(), "reporting_days": $('#reporting_days').val() },
		dataType: 'JSON',
